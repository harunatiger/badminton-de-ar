...
(function (window, document) {
	...
	Croppic.prototype = {
		...
		init: function () {
			...
			*/
      var client = $('.client').css('content');
      var desktop = (client == undefined || client == '');
      if(desktop){
        that.objW = 300;
        that.objH = 300;
      }else{
			...
			*/
            ...
            var file = $(this).prop('files')[0];
              ...
              if(!IsImage(file.name)){
                ...
                $.ajax({
                  type: 'POST',
                  url: '/profiles/' + that.options.cropData['profile_id'] + '/profile_images/check_validation',
                  data: { 'image': e.target.result}
                });
                return false
              };
              
							var image_buf = new Image();
              var image = new Image();
              
              image_buf.src = e.target.result;
							image_buf.onload = function(){
                EXIF.getData(image_buf, function() {
                  var orientation = image_buf.exifdata.Orientation;
                  var mpImg = new MegaPixImage(image_buf);
                  mpImg.render(image, { orientation: orientation });
                });
              };
      ...
      wrapper = $('.cropImgWrapper');
      wrapper.after(that.options.loaderHtml);
			that.loader = that.obj.find('.loader');
